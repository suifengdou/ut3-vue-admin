<template>
  <div class="inbound-submit-container">
        ...
        <el-col :span="5" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="快捷搜索" placement="top-start">
              <el-input v-model="params.warehouse__name" class="grid-content bg-purple" placeholder="请输入仓库名称" @keyup.enter.native="fetchData">
              ...
              <el-collapse-item>
                ...
                <div class="block">
                  <el-form ref="filterForm" :model="params" label-width="80px">
                    ...
                    <el-row :gutter="20">
                      <el-col :span="6"><el-form-item label="验证号" prop="verification">
                        <el-input v-model="params.verification" type="text" />
                      </el-form-item></el-col>
        ...
        >
          <template slot-scope="scope">
            <div v-for="(item, i) in scope.row.goods_details">
              <el-tag type="success" size="mini" effect="dark"><span>{{ item.goods.name }}*{{ item.quantity }}</span></el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8"><el-form-item label="入库类型" prop="category">
              <el-select v-model="formAdd.category" placeholder="请选择入库类型">
                <el-option
          ...
          <el-row :gutter="20">
            <el-col :span="8"><el-form-item label="验证号" prop="verification">
              <el-input v-model="formAdd.verification" placeholder="请输入验证号" />
            </el-form-item></el-col>
            <el-col :span="8"><el-form-item label="体积" prop="volume">
              <el-input v-model="formAdd.volume" placeholder="请输入体积" />
            </el-form-item></el-col>
        ...
        <el-card class="box-card">
          ...
          >
            ...
            <el-table-column label="名称" width="250" prop="goods">
              <template slot-scope="scope">
                <el-select
                  v-model="OrderDetailsList[scope.row.xh-1].goods"
                  filterable
                  default-first-option
                  remote
                  reserve-keyword
          ...
          >
            <el-card class="box-card">
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="8"><el-form-item label="单据类型" prop="category">
                  <el-select v-model="formEdit.category" placeholder="请选择单据类型">
                    <el-option
                      v-for="item in optionsCategory"
                      :key="item.value"
                      :label="item.label"
              ...
              <el-row :gutter="20">
                ...
                <el-col :span="8"><el-form-item label="体积" prop="volume">
                  <el-input v-model="formEdit.volume" placeholder="请输入体积" />
                </el-form-item></el-col>
                <el-col :span="8"><el-form-item label="重量" prop="weight">
            ...
            <el-card class="box-card">
              ...
              >
                ...
                <el-table-column label="名称" width="250" prop="goods">
                  <template slot-scope="scope">
                    <el-select
                      v-model="OrderDetailsListEdit[scope.row.xh-1].goods"
                      filterable
                      default-first-option
                      remote
                      reserve-keyword
                ...
                <el-table-column label="货品数量" width="250" prop="quantity">
                  <template slot-scope="scope">
                    <el-input v-model="OrderDetailsListEdit[scope.row.xh-1].quantity" type="number" />
                  </template>
                </el-table-column>
                <el-table-column label="含税单价" width="250" prop="price">
                  <template slot-scope="scope">
                    <el-input v-model="OrderDetailsListEdit[scope.row.xh-1].price" type="text" />
                  </template>
                </el-table-column>
                <el-table-column label="货品备注" width="250" prop="memo">
                  <template slot-scope="scope">
...
import {
  ...
  updateInboundSubmit,
  exportInboundSubmit,
  excelImportInboundSubmit,
  checkInboundSubmit,
  rejectInboundSubmit
} from '@/api/psi/inbound/order/submit'
import {getLogInboundManage} from "@/api/psi/inbound/order/manage"
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getInboundSubmit(this.params).then(
        res => {
    ...
    handleEdit(values) {
      ...
      this.optionsGoods = this.formEdit.goods_details.map(item => {
        return { label: item.goods.name, value: item.goods.id }
      })
      ...
      for (index in this.formEdit.goods_details) {
        ...
        this.formEdit.goods_details[index].goods = this.formEdit.goods_details[index].goods.id
        this.OrderDetailsListEdit.push(this.formEdit.goods_details[index])
      }
    ...
    handleSubmitEdit() {
      ...
      data.goods_details = this.OrderDetailsListEdit
      console.log(this.formEdit)
      updateInboundSubmit(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createInboundSubmit(this.formAdd).then(
        () => {
          this.fetchData()
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportInboundSubmit(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportInboundSubmit(this.params).then(
              res => {
    ...
    handleFix() {
      ...
      if (this.params.allSelectTag === 1) {
        fixInboundSubmit(this.params).then(
          res => {
    ...
    handleCheck() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        checkInboundSubmit(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectInboundSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
