<template>
  <div class="storage-check-container">
...
export default {
  ...
  methods: {
    ...
    handleSubmitAdd() {
      ...
      createWorkOrderCheck(this.formAdd).then(
        () => {
          this.fetchData()
    ...
    importExcel() {
      ...
      excelImportWorkOrderCheck(importformData, config).then(
        res => {
          this.$notify({
            title: '导入结果',
            message: res.data,
