<template>
  <div class="label-customer-center-container">
...
<script>
import { 
  getLabelCustomerCenter,
  createLabelCustomerCenter,
  updateLabelCustomerCenter,
  exportLabelCustomerCenter,
  excelImportLabelCustomerCenter,
  checkLabelCustomerCenter,
  rejectLabelCustomerCenter
 } from '@/api/crm/labels/labelcustomer/center' 
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getLabelCustomerCenter(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateLabelCustomerCenter(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createLabelCustomerCenter(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportLabelCustomerCenter(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportLabelCustomerCenter(this.params).then(
              res => {
    ...
    handleCheck() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        checkLabelCustomerCenter(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectLabelCustomerCenter(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
