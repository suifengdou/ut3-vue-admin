...
import {
  ...
  updateLabelCategory,
  exportLabelCategory,
  excelImportLabelCategory,
  getLogLabelCategory
} from '@/api/crm/labels/label/labelcategory'
import { getCompanyList } from '@/api/base/company'
import moment from 'moment'
import XLSX from 'xlsx'
export default {
  name: 'submitExpressWorkLabelCategory',
  data() {
    return {
      DataList: [],
      tableLoading: false,
  ...
  methods: {
    fetchData() {
      ...
      getLabelCategory(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateLabelCategory(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createLabelCategory(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportLabelCategory(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportLabelCategory(this.params).then(
              res => {
    ...
    handleFix() {
      ...
      if (this.params.allSelectTag === 1) {
        fixLabelCategory(this.params).then(
          res => {
    ...
    handleCheck() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        checkLabelCategory(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectLabelCategory(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
