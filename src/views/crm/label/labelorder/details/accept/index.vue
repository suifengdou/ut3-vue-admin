...
export default {
  ...
  methods: {
    ...
    handleSubmitAdd() {
      ...
      createLabelCustomerOrderDetailsSubmit(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportLabelCustomerOrderDetailsSubmit(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportLabelCustomerOrderDetailsSubmit(this.params).then(
              res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            const wb = XLSX.utils.book_new()
            XLSX.utils.book_append_sheet(wb, ws, '数据详情')
            XLSX.writeFile(wb, '客户标签明细模板1.xlsx')
            resultMessage = '表格导出成功啦'
            resultType = 'success'
            instance.confirmButtonLoading = false
            done()
    ...
    handleCheck() {
      this.tableLoading = true
      if (this.params.allSelectTag === 1) {
        checkLabelCustomerOrderDetailsSubmit(this.params).then(
          res => {
            if (res.data.successful !== 0) {
              this.$notify({
                title: '审核成功',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectLabelCustomerOrderDetailsSubmit(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '取消成功',
    ...
    logView(userValue) {
      ...
      getLogLabelCustomerOrdeDetails(data).then(
        res => {
          this.$notify({
            title: '查询成功',
            type: 'success',
