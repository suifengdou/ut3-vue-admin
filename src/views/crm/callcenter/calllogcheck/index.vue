<template>
  <div class="calllog-check-container">
...
import {
  ...
  checkCallLogCheck,
  rejectCallLogCheck
} from '@/api/crm/callcenter/calllogcheck'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getCallLogCheck(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateCallLogCheck(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createCallLogCheck(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportCallLogCheck(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportCallLogCheck(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkCallLogCheck(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectCallLogCheck(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
