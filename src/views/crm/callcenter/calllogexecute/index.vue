<template>
  <div class="calllog-execute-container">
...
import {
  getCallLogExecute,
  createCallLogExecute,
  updateCallLogExecute,
  exportCallLogExecute,
  checkCallLogExecute,
  rejectCallLogExecute
} from '@/api/crm/callcenter/calllogexecute'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getCallLogExecute(this.params).then(
        res => {
    ...
    handleSubmitEdit() {
      ...
      updateCallLogExecute(id, data).then(
        () => {
          this.$notify({
            title: '修改成功',
            type: 'success',
    ...
    handleSubmitAdd() {
      ...
      createCallLogExecute(this.formAdd).then(
        () => {
          this.$notify({
            title: '创建成功',
            type: 'success',
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportCallLogExecute(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true
            instance.confirmButtonText = '执行中...'
            exportCallLogExecute(this.params).then(
              res => {
    ...
    handleCheck() {
      ...
      if (this.params.allSelectTag === 1) {
        checkCallLogExecute(this.params).then(
          res => {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            if (this.params.allSelectTag === 1) {
              rejectCallLogExecute(this.params).then(
                res => {
                  if (res.data.successful !== 0) {
                    this.$notify({
                      title: '驳回成功',
