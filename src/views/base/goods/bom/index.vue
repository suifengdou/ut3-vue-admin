<template>
  <div class="goods-bom-container">
    <div class="tableTitle">
      <el-row :gutter="20">
        <el-col :span="5" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="货品名称" placement="top-start">
              <el-input v-model="params.goods__name" class="grid-content bg-purple" placeholder="请输入货品名称" @keyup.enter.native="fetchData">
        ...
        <el-col :span="5" class="titleBar">
          <div class="grid-content bg-purple">
            <el-tooltip class="item" effect="dark" content="配件名称" placement="top-start">
      ...
      >
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8"><el-form-item label="配件" prop="part">
              <template>
                <el-select
                  v-model="formAdd.part"
                  filterable
                  default-first-option
                  remote
                  reserve-keyword
                  placeholder="请搜索并选择配件"
                  :remote-method="remoteMethodPart"
                >
                  <el-option
...
<script>
import { getBom, createBom, updateBom, getLogBom } from '@/api/base/bom'
...
export default {
  ...
  methods: {
    ...
    handleEdit(values) {
      ...
      this.optionsPart = [{ label: this.formEdit.part.name, value: this.formEdit.part.id }]
      this.formEdit.part = this.formEdit.part.id
    ...
    remoteMethodPart(query) {
      console.log(query)
      if (query !== '') {
        setTimeout(() => {
          const paramsSearch = {}
          paramsSearch.name = query
          paramsSearch.goods_attribute = 2
          getGoodsList(paramsSearch).then(
            res => {
              this.optionsPart = res.data.results.map(item => {
                return { label: item.name, value: item.id }
              })
            }
          )
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            excelImportBom(importformData, config).then(
              res => {
                this.$notify({
                  title: '导入结果',
                  message: res.data,
